...
<br>
- 현재 EC2 t3.medium에서 배포 중이며 프론트엔드 서버, API 서버(WAS), NGINX(웹 서버)로 구성되어 있습니다.
- 모든 서버는 컨테이너로 분리돼 배포 중이며 도커 bridge 네트워크에서 http 통신을 통해 API를 주고 받습니다.
...
#### 프론트엔드 서버
- 클라이언트로부터 요청받은 정적 페이지를 로드 합니다.
- API 매니저를 통해 UI/UX에 맞는 API를 백엔드 서버로 요청합니다.
#### API 서버
#### 인증/인가 서버 (Auth)
